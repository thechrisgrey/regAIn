...
- **DynamoDB PITR**: Point-in-time recovery enabled on all 7 data tables (UserProfiles, Campaigns, MissionHistory, EvidenceVault, MarketData, VoiceSessions, ConversationThreads). NOT on WebSocketConnections (ephemeral, TTL-based)
...
- **Amplify env var timing**: Env vars are resolved at build **start** time. If you update env vars after a build triggers, the current build won't have them — trigger a fresh rebuild with `aws amplify start-job --job-type RELEASE --branch-name main --app-id d2z52fw5cbbzo`
- **Amplify `update-app --environment-variables` REPLACES all vars**: This is NOT a merge — passing one var wipes every other var. Always pass the complete set. Use `aws amplify get-app --query 'app.environmentVariables'` to read current vars first, then include all of them in the update
