...
- **Coaching chat streaming**: `useStreamingCoaching` hook connects via WebSocket (`VITE_CHAT_WS_URL`) for progressive text responses. `MarkdownMessage` component renders assistant markdown via `react-markdown` + `remark-gfm`. Hook exposes `thinking` + `thinkingLabel` states for tool execution visibility
- **Coaching tool execution hooks**: `stream_handler.py` uses Strands `HookProvider` (`_StreamingToolHooks`) to send `{"type": "thinking", "tool": "<name>"}` WebSocket events on `BeforeToolCallEvent`. This prevents the frontend from appearing stuck during DynamoDB reads and other tool calls. A safety `threading.Timer` also fires 10s before Lambda timeout to send an error before the process is killed
