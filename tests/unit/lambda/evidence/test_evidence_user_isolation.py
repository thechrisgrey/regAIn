...
# DynamoDBClient has no 'backend.lambda' imports so importlib works.
_dynamodb_mod = importlib.import_module("backend.lambda.shared.dynamodb")
DynamoDBClient = _dynamodb_mod.DynamoDBClient

# Evidence service imports 'backend.lambda.shared...' (syntax error due
# to the 'lambda' keyword), so we verify it via source text instead.
_SERVICE_PATH = (
